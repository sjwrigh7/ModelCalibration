...
This implementation is for a multivariate normal distribution data model.
...
Function to draw a sample of θ's posterior distribution using the Metropolis-Hastings algorithm.
This implementation is for a univariate normal distribution data model.
...
"""
function metropolis_theta(model,prior_data::PriorData,data::DataStr,
            theta::Vector{Float64},sig2::Float64,k::Int64,stepsize::Float64)
    c1 = 1.0              #scale of theta
    c2 = 0.0              #min value for theta
...
function metropolis_rho(prior_data::PriorData,data::DataStr,
            ...
            k::Int64,stepsize::Float64,nx::Int64,nloc::Int64)
    ...
    corr_current = correlation_construct(rho,data.exp.x,nx,nloc)
    log_delta_prop = log_prior_delta(delta,corr_current,tau2)[1]
    rho[k] = prop_rho               #replace rho at k with prop val
    corr_prop = correlation_construct(rho,data.exp.x,nx,nloc)
...
---
Details
The full conditional distribution of τ^2 is proportional to a multivariate normal distribution (the discrepancy term's prior distribution) multiplied by an inverse gamma distribution (the τ^2 prior distribution).
This can be simplified to a well known inverse gamma posterior distribution form for Gibbs updates.
Given prior distribution parameters for τ^2 of IG(α,β) and prior distribution for δ of MVN(0,τ^2*C), the posterior distribution is solved as the following:
p(τ^2|.) ∼ IG(α+n/2,β+0.5*δ'*C^(-1)*δ)
Where n is the length of the discrepancy term (the same length as the data model's multivariate normal distribution).
"""
function gibbs_tau2(prior_data::PriorData,delta::Vector{Float64},
            corr::Array{Float64,2},nloc::Int64)
...
---
Details
The full conditional distribution of σ^2 is proportional to a multivariate normal likelihood function multiplied by an inverse gamma distribution (the σ^2 prior distribution).
This can be simplified to a well known inverse gamma posterior distribution form for Gibbs updates.
Given prior distribution parameters for σ^2 of IG(α,β) and likelihood function of ∏MVN(η+δ,σ^2*I), the posterior distribution is solved as the following:
...
Given p(δ) = MVN(0,Σ) and L(.|x,y) = ∏MVN(η+δ,σ^2*I), the posterior distribution for δ can be solved as the following:
p(δ|.) ∼ MVN(bn*An,An)
where
An = (Σ^-1 + m^2(σ^2*I)^-1)^-1
bn = m^2((σ^2*I)^-1)*(ȳ-η)
...
function gibbs_delta(data::DataStr,sig2::Float64,tau2::Float64,
    ...
    sample = rand(MvNormal(vec(covar*bn),covar))  #sample from posterior
    return sample
end
