...
function lik( data::DataStr,vars::UpdatedVars,
    ...
    covariance = sig2*(Matrix(1.0I,sz,sz)+vars.tau2*correlation_construct(vars.rho,data.exp.x,nx,nobs))              #calculate covar matrix
...
function metropolis_tau2(prior_data::PriorData,data::DataStr,vars::UpdatedVars,
    stepsize::Float64,nx::Int64,nobs::Int64)
    c1 = 10.0              #rho scale
    c2 = 1e-10              #rho min val
    current_tau2 = vars.tau2     #pull current rho at index k
    current_gamma = g_inv(c1,c2,current_tau2) #transform to gamma
    prop_gamma = rand(Normal(current_gamma,stepsize)) #propose new gamma
    prop_tau2 = g(c1,c2,prop_gamma)           #transform back to rho

    #vars_prop = copy(vars)
    #ars_prop.tau2 = prop_tau2
    lik_current = lik(data,vars,vars.theta)[1]*pdf(InverseGamma(1e-9,1e-9),vars.tau2)
    vars.tau2 = prop_tau2
    lik_prop = lik(data,vars,vars.theta)[1]*pdf(InverseGamma(1e-9,1e-9),prop_tau2) #calc likelihood
    
    #calculate jump distribution values
    jump_current = pdf(Normal(current_gamma,stepsize),prop_gamma)*
    abs(-(c1)/((prop_tau2+c2)*(-c1+prop_tau2+c2)))
    jump_propose = pdf(Normal(prop_gamma,stepsize),current_gamma)*
    abs(-(c1)/((current_tau2+c2)*(-c1+current_tau2+c2)))  
    #calculate acceptance
    ratio = min((lik_prop/lik_current)*jump_propose/jump_current, 1)
    accept = rand(Uniform(0,1))<ratio   
    new_value = ifelse(accept,prop_tau2,current_tau2) #determine acceptance   
    output = MetropolisInfo(new_value,ratio,accept)
    vars.tau2 = new_value
   return output
end
...
function mcmc(data::DataStr,nmcmc::Int64,prior_data::PriorData,
    ...
    if size(data.exp.x)[2] == 0
        ...
        @inbounds @showprogress 1 "Computing..." for i in start:stop
            ...
            bulk_vars.sig2[i] = step_vars.sig2[1,1]
            #gibbs update for sig^2
            tau2_step = metropolis_tau2(prior_data,data,step_vars,  
            1e-1,nx,nobs)
            step_vars.tau2 = tau2_step.new_value
            bulk_vars.tau2[i] = step_vars.tau2
            #bulk_vars.accept[i,end] = tau2_step.accept
            #bulk_vars.ratio[i,end] = tau2_step.ratio
...
function post(results::BulkVarsStruct,nx::Int64,ntheta::Int64,nburn::Int64,
    ...
    results.tau2 = sqrt.(results.tau2) .* results.sig2
